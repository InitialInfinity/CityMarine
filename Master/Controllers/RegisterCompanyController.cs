...
namespace Master.Controllers
{
	...
	public class RegisterCompanyController : ControllerBase
	{
        ...
        public RegisterCompanyController(ICompanyDetailsRepository CRepo, ISubscriptionMasterRepository subRepo)
        {
            _CRepo = CRepo;
            _subRepo = subRepo;
        }
		...
		[HttpPost("InsertSub")]
        public async Task<IActionResult> InsertSub([FromBody] Subscription sub)
		{
			try 
			{
                if (sub.BaseModel == null)
                {
                    sub.BaseModel = new BaseModel();
                }
                sub.BaseModel.OperationType = "InsertSubscription";
                var parameter = await _CRepo.SubDetails(sub);
				return parameter;
            }
			catch (Exception)
			{
        ...
        public async Task<IActionResult> InsertPayment([FromBody] PaymentDetails com)
        {
            try
            {
                if (com.BaseModel == null)
                ...
                com.BaseModel.OperationType = "InsertPaymentDetails";
                var parameter = await _CRepo.PayDetails(com);
                return parameter;
            }
            catch (Exception)
            {
        ...
        public async Task<IActionResult> GetSubscription()
        {
            try
            {
                ...
                user.BaseModel.OperationType = "GetAllSubcription";
