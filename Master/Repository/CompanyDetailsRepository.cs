...
namespace Master.Repository
{
	public class CompanyDetailsRepository : ICompanyDetailsRepository
	{
        ...
        public async Task<IActionResult> SubDetails(Subscription user)
		{
            using (var connection = _context.CreateConnection())
            {
                //Guid? userIdValue = (Guid)user.UserId;
                try
                {
                    var sqlConnection = (Microsoft.Data.SqlClient.SqlConnection)connection;
                    await sqlConnection.OpenAsync();
                    var queryResult = await connection.QueryMultipleAsync("proc_CompanyRegistration", SetSub(user), commandType: CommandType.StoredProcedure);
                    var Model = queryResult.ReadSingleOrDefault<Object>();
                    var outcome = queryResult.ReadSingleOrDefault<Outcome>();
                    var outcomeId = outcome?.OutcomeId ?? 0;
                    var outcomeDetail = outcome?.OutcomeDetail ?? string.Empty;
        ...
        public DynamicParameters SetSub(Subscription user)
        {
            DynamicParameters parameters = new DynamicParameters();
            parameters.Add("@OperationType", user.BaseModel.OperationType, DbType.String);
            parameters.Add("@com_id", user.com_id, DbType.Guid);
            parameters.Add("@sm_id", user.sm_id, DbType.Guid);
            //parameters.Add("@UserId", user.com_id, DbType.Guid);            
            parameters.Add("@com_company_name", user.com_company_name, DbType.String);
        ...
        public async Task<IActionResult> PayDetails(PaymentDetails user)
        {
            using (var connection = _context.CreateConnection())
            {
                //Guid? userIdValue = (Guid)user.UserId;
                try
                {
                    var sqlConnection = (Microsoft.Data.SqlClient.SqlConnection)connection;
                    await sqlConnection.OpenAsync();
                    var queryResult = await connection.QueryMultipleAsync("proc_CompanyRegistration", SetPay(user), commandType: CommandType.StoredProcedure);
                    var Model = queryResult.ReadSingleOrDefault<Object>();
                    var outcome = queryResult.ReadSingleOrDefault<Outcome>();
                    var outcomeId = outcome?.OutcomeId ?? 0;
                    var outcomeDetail = outcome?.OutcomeDetail ?? string.Empty;
        ...
        public DynamicParameters SetPay(PaymentDetails user)
        {

            DynamicParameters parameters = new DynamicParameters();
            parameters.Add("@OperationType", user.BaseModel.OperationType, DbType.String);
            parameters.Add("@com_id", user.com_id, DbType.Guid);
            parameters.Add("@sub_id", user.sub_id, DbType.Guid);
            parameters.Add("@com_company_name", user.com_company_name, DbType.String);
            parameters.Add("@com_email", user.com_email, DbType.String);
            parameters.Add("@Amount", user.Amount, DbType.String);
            parameters.Add("@Payment_Mode", user.Payment_Mode, DbType.String);
