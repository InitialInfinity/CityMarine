...
namespace ibillcraft.Controllers
{
    public class InboxClientController : Controller
    {
        ...
        private readonly ILogger<InboxClientController> _logger;
        private readonly IStringLocalizer<InboxClientController> _localizer;
        public InboxClientController(ILogger<InboxClientController> logger, IStringLocalizer<InboxClientController> localizer, IConfiguration configuration)
        {
        ...
        public async Task<JsonResult> showdetails(string id)
        {
            ...
            var sentclientDataList = new InboxClientModel();
            var sentclientList = new InboxClientModel();
            string sentclienturl = $"{_httpClient.BaseAddress}/InboxClient/Get?UserId={UserId}&ic_id={id}";
            //HttpResponseMessage response = _httpClient.GetAsync(sentclienturl).Result;
            HttpResponseMessage response = await _httpClient.GetAsync(sentclienturl);
            if (response.IsSuccessStatusCode)
            {
                //dynamic data = response.Content.ReadAsStringAsync().Result;
        ...
        public JsonResult general(string tab)
        {
            ...
            var sentclientDataList = new List<InboxClientModel>();
            var sentclientList = new List<InboxClientModel>();
            string sentclienturl = $"{_httpClient.BaseAddress}/InboxClient/GetGeneral?UserId={UserId}&ic_type={tab}";
            HttpResponseMessage response = _httpClient.GetAsync(sentclienturl).Result;
            if (response.IsSuccessStatusCode)
            {
                dynamic data = response.Content.ReadAsStringAsync().Result;
