...
namespace ibillcraft.Controllers
{
	...
	public class CustomerMasterController : Controller
	{
		...
		//public IActionResult Create([FromForm] CustomerMasterModel model)
		{
			try
			{
				GetCookies gk = new GetCookies();
				...
				var orderModel = new List<CustomerMasterModel>();
				...
				string url = $"{_httpClient.BaseAddress}/CustomerMaster/GetDomain?UserId={UserId}&Server_Value={CUtility.serverValue}&comid={CUtility.comid}&c_domain={model.c_domain}";
				...
				if (response1.IsSuccessStatusCode)
				{
					...
					if (orderModel.Count > 0)
					{
						...
						//TempData["errorMessage"] = "One or more domains already exist";
						//return Json(new { success = false, redirectUrl = Url.Action("EditIndex1"), message = TempData["errorMessage"] });
