    ...
    public class UserMasterController : Controller
    {
        ...
        public IActionResult Index(string status)
        {
            try
            {
                ...
                //            string n = $"{_httpClient.BaseAddress}/ViewBag/GetViewBag?userId={CUtility.comid}&sTableName=tbl_Staff&sValue=st_staff_name&id=st_id&IsActiveColumn=st_isactive&Server_Value={CUtility.serverValue}&sCoulmnName=st_com_id&sColumnValue={CUtility.comid}";
                ...
                string n = $"{_httpClient.BaseAddress}/UserMaster/GetStaff?UserId={UserId}&Server_Value={CUtility.serverValue}";
                HttpResponseMessage nView = _httpClient.GetAsync(n).Result;
                dynamic namedata = nView.Content.ReadAsStringAsync().Result;
                ...
                string usermasterurl = $"{_httpClient.BaseAddress}/UserMaster/GetAll?UserId={UserId}&status={status}&Server_Value={CUtility.serverValue}";
        ...
        public IActionResult Create(UserMasterModel model)
        {
            try
            {
				...
				model.UserId = UserId;
                model.um_com_id = CUtility.comid;
        ...
        public IActionResult Username(UserMasterModel model)
        {
            try
            {
                ...
                string data = JsonConvert.SerializeObject(model);
                StringContent content = new StringContent(data, Encoding.UTF8, "application/json");
                string url = $"{_httpClient.BaseAddress}/Staff/Get?st_id={staffid}&UserId={UserId}&Server_Value={CUtility.serverValue}&st_com_id={new Guid(CUtility.comid)}";
        ...
        public IActionResult Edit(Guid? um_id)
        {
            ...
            try
            {
                ...
                string url = $"{_httpClient.BaseAddress}/UserMaster/Get?um_id={um_id}&UserId={UserId}&Server_Value={CUtility.serverValue}";
                HttpResponseMessage response = _httpClient.GetAsync(url).Result;
                if (response.IsSuccessStatusCode)
                {
                    dynamic data = response.Content.ReadAsStringAsync().Result;
        ...
        public async Task<IActionResult> Excel( string status)
        {
            try
            {
                ...
                Guid? UserId = new Guid(CUtility.comid);
                model.Server_Value = CUtility.serverValue;
                string url = $"{_httpClient.BaseAddress}/UserMaster/GetExcel?UserId={UserId}&status={status}&Server_Value={CUtility.serverValue}";
        ...
        public async Task<IActionResult> Pdf( string status)
        {
            ...
            try
            {
                ...
                model.Server_Value = CUtility.serverValue;
                string data = JsonConvert.SerializeObject(model);
                string url = $"{_httpClient.BaseAddress}/UserMaster/GetPdf?UserId={UserId}&status={status}&Server_Value={CUtility.serverValue}";
                HttpResponseMessage response = await _httpClient.GetAsync(url);
