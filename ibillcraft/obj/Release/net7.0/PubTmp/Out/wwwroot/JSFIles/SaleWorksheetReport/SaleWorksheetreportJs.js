...
function report(customer, fromdate, todate) {
    ...
    $.ajax({
        type: 'POST',
        url: 'saleworksheetreport/Report?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate + '', // Use the form's action attribute
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.data.length; i++) {
                htmlrpt += '<tr><td>' + response.data[i].sl_invoice_date + '</td><td>' + response.data[i].si_product_name + '</td><td style="text-align:right">' + response.data[i].si_size + '</td><td style="text-align:right">' + response.data[i].si_rate + '</td><td style="text-align:right">' + response.data[i].si_qty + '</td><td style="text-align:right">' + response.data[i].si_cgst + '</td><td style="text-align:right">' + response.data[i].si_sgst + '</td><td style="text-align:right">' + response.data[i].si_igst + '</td><td style="text-align:right">' + response.data[i].si_ugst + '</td><td style="text-align:right">' + response.data[i].si_samount + '</td></tr>';
            }
