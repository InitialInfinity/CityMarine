function addorder() {
    window.location.href = "/Proforma/EditIndex";
}
...
function submitProforma(type) {
    /* event.preventDefault();*/
/*    var test = ('#qu_dtp_charges').val();*/
    ...
    if ($('#qu_c_id').val() == '') {
        ...
        if ($('#qu_order_ref').val() == '' || $('#qu_order_ref').val() == '--Select--') {
    ...
    var data = {
        ...
        qu_total_withoutgst: parseFloat($('#qu_total_withoutgst').val()),
        ...
        ProformaInvoice: orderDataArray
    };
    $.ajax({
        type: 'POST',
        url: '/Proforma/Create', // Use the form's action attribute
...
function customer(qu_c_id) {
    ...
    $.ajax({
        type: 'POST',
        url: '/Proforma/Customer', // Use the form's action attribute
        data: data, // Serialize the form data
        success: function (response) {
...
function gst() {
    ...
    $("#qu_gst").val((parseFloat(gst.toFixed(2))));
...
function final_total() {
    ...
    var sub = ($("#qu_total_withoutgst").val());
    ...
    if (advance == "0" && discount == "0") {
        ...
        var roundoff = (parseFloat(balance.toFixed(2)));
        $("#qu_balance").text(roundoff.toFixed(2));
        $("#qu_total_withgst").val((parseFloat(totalwithgst.toFixed(2))));
...
function Excel() {
    window.location.href = '/Proforma/Excel';
}

function Pdf() {
    window.location.href = '/Proforma/Pdf';
}
...
//$("#qu_invoice_date").datepicker({
//    dateFormat: "dd/mm/yyyy",
//    defaultDate: new Date()
//});
//$("#qu_due_date").datepicker({
//    dateFormat: "dd/mm/yyyy",
//    defaultDate: new Date() + 10
//});
function Excel12(customer, fromdate, todate, staff) {
    ...
    window.location.href = '/ProformaReport/Excel?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate + '&staff=' + sstaff +'';
...
function report(customer, fromdate, todate, staff) {
    ...
    $.ajax({
        type: 'POST',
        url: 'ProformaReport/Report?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate + '&staff=' + sstaff +'', // Use the form's action attribute
        //data: data, // Serialize the form data
        success: function (response) {
            ...
            for (var i = 0; i < response.length; i++) {
                htmlrpt += '<tr><td>' + (parsefloat(i) + 1) + '</td><td style=display:none>' + response[i].pr_id + '</td><td>' + response[i].qu_invoice_no + '</td><td>' + response[i].qu_invoice_date + '</td><td>' + response[i].qu_name + '</td><td>' + response[i].qu_order_ref + '</td><td  style="text-align:right">' + response[i].qu_total_withgst + '</td><td><div class="d-inline-flex"><a href = "/Proforma/BillIndex?qu_invoice_no=' + response[i].qu_invoice_no + '&amp;qu_name=' + response[i].qu_name + '&pr_id=' + response[i].pr_id + '&qu_total_withoutgst=' + response[i].qu_total_withoutgst + '&qu_total_withgst=' + response[i].qu_total_withgst + '&qu_total_gst=' + response[i].qu_total_gst + '&amp;qu_advance=' + response[i].qu_advance + '&qu_balance=' + response[i].qu_balance + '&total_charges=' + response[i].totalcharges + '"><span class="fa fa-eye"></span></a></div></td></tr>';
            }
