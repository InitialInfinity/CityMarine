function report(customer, fromdate, todate, sp_mode) {
    ...
    $.ajax({
        type: 'POST',
        url: 'SalePaymentModeReport/Report?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate + '&sp_mode=' + ssp_mode + '', // Use the form's action attribute
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                htmlrpt += '<tr><td>' + (parsefloat(i) + 1) + '</td><td>' + response[i].sl_invoice_no + '</td><td>' + response[i].sl_invoice_date + '</td><td>' + response[i].sl_name + '</td><td><span class="badge py-3 px-4 fs-7 ' + color + '">' + status + '</span></td><td>' + response[i].sl_payment_method + '</td><td style="tetx-align:right">' + response[i].sl_advance + '</td><td style="tetx-align:right">' + response[i].sl_balance + '</td><td style="tetx-align:right">' + response[i].sl_total_withgst + '</td><td><div class="d-inline-flex"><a href = "/SaleOrder/BillIndex?sl_invoice_no=' + response[i].sl_invoice_no + '&amp;sl_name=' + response[i].sl_name + '&amp;sl_id=' + response[i].sl_id + '&amp;sl_total_withgst=' + response[i].sl_total_withgst + '&amp;sl_advance=' + response[i].sl_advance + '&amp;sl_balance=' + response[i].sl_balance + '&amp;total_charges=0' + '"><span class="fa fa-eye"></span></a></div></td></tr>';
            }
