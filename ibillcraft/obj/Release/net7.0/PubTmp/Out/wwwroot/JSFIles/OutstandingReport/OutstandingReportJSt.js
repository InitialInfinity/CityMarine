function report(customer, fromdate, todate) {
    ...
    $.ajax({
        type: 'POST',
        url: 'OutstandingReport/Report?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate  + '', // Use the form's action attribute
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                htmlrpt += '<tr><td>' + (parsefloat(i) + 1) + '</td><td style="display:none">' + response[i].sl_id + '</td><td>' + response[i].sl_invoice_no + '</td><td>' + response[i].sl_invoice_date + '</td><td>' + response[i].sl_name + '</td><td><span class="badge py-3 px-4 fs-7 ' + color + '">' + response[i].status + '</span></td><td style="text-align:right">' + response[i].sl_balance + '</td><td style="text-align:right">' + response[i].sl_total_withgst + '</td></tr>';
            }
...
function Excel(customer, fromdate, todate) {
    ...
    window.location.href = '/OutstandingReport/Excel?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate + '';
}