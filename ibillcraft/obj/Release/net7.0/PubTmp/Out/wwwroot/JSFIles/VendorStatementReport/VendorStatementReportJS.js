function report(customer, fromdate, todate) {
    ...
    $.ajax({
        type: 'POST',
        url: 'VendorStatementReport/Report?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate + '', // Use the form's action attribute
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.data.length; i++) {
                htmlrpt += '<tr><td>' + (parsefloat(i) + 1) + '</td><td>' + response.data[i].pcp_invoice + '</td><td>' + response.data[i].pcp_date + '</td><td style="text-align:right">' + response.data[i].pcp_due + '</td><td style="text-align:right">' + response.data[i].pcp_pay + '</td><td>' + response.data[i].pcp_mode + '</td><td style="text-align:right">' + response.data[i].pcp_balance + '</td></tr>';
            }
            $('#ccontact').text(response.data[0].v_contact);
            $('cgst').text(response.data[0].v_gst_no);
...
function Excel(customer, fromdate, todate) {
    ...
    var sfromdate = $('#from').val();
    var stodate = $('#to').val();
    window.location.href = '/VendorStatementReport/Excel?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate + '';
}