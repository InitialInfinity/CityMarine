function addsale() {
    window.location.href = "/SaleOrder/EditIndex1";
}
...
function submitSaleorder(type) {
    ...
    $("#dataTable tbody tr:gt(0)").each(function () {
        var rowData = {
            ...
            si_desc: $(this).find('td:eq(2)').text(),
            si_product_name: $(this).find('td:eq(3)').text(),
    ...
    $.ajax({
        ...
        success: function (response) {
            if (type == 'Close') {
                window.location.href = "/SaleOrder/Index1";
            }
            else {
                window.location.href = "/SaleOrder/EditIndex1";
            }
...
function submitgrid() {
    ...
    var newRow = "<tr style='text-align: right;'>" +
        ...
        "<td style='display:none;'>" + sl_desc + "</td>" +
...
$(document).on('click', '#dataTable tbody .click-edit', function () {
    ...
    var sl_desc = $rowToEdit.find('td:eq(2)').text();
    var sl_product_name = $rowToEdit.find('td:eq(3)').text();
...
function payment() {
    ...
    //}
    //else {
    window.location.href = '/SaleOrder/Payment1?sl_invoice_no=' + sl_invoice_no + '&sl_cust_name=' + sl_cust_name + '&sl_balance=' + sl_balance + '&sl_cust_id=' + sl_cust_id + "&sl_id=" + sl_id + "&type=" + paymentmethod;
    //}
}
function payamount() {
...
function submitpayment() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            window.location.href = '/SaleOrder/Payment1?sl_invoice_no=' + response.sp_invoice_no + '&sl_cust_name=' + response.sp_cust_name + '&sl_balance=' + response.sp_balance + '&sl_cust_id=' + response.sp_cust_id + '&type=' + response.sp_PaymentType + '&sl_id=' + response.sp_invoice_id;
