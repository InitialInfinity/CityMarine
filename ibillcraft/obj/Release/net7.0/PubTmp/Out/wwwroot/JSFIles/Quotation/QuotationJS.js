function addorder() {
    window.location.href = "/Quotation/EditIndex";
}
function validateDueDate() {
    ...
    var currentDateInput = $("#qu_invoice_date");
...
function addproforma() {
    window.location.href = '@Url.Action("EditQuot", "Proforma")';
}
function Excel12(customer, fromdate, todate, staff) {
    var scustomer = $('#customer').val();
    var sfromdate = $('#from').val();
    var stodate = $('#to').val();
    var sstaff = $('#order_ref').val();

    window.location.href = '/QuotationReport/Excel?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate + '&staff=' + sstaff + '';

}
function reportquotation(customer, fromdate, todate, staff) {
    ...
    $.ajax({
        type: 'POST',
        url: 'QuotationReport/Report?customer=' + scustomer + '&fromdate=' + sfromdate + '&todate=' + stodate + '&staff=' + sstaff + '', // Use the form's action attribute
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.length; i++) {
                htmlrpt += '<tr><td>' + (parsefloat(i) + 1) + '</td><td>' + response[i].qu_invoice_no + '</td><td>' + response[i].qu_invoice_date + '</td><td>' + response[i].qu_name + '</td><td>' + response[i].qu_order_ref + '</td><td style="text-align:right">' + response[i].qu_total_withgst + '</td><td><div class="d-inline-flex"><a href = "/Quotation/BillIndex?qu_invoice_no=' + response[i].qu_invoice_no + '&amp;qu_name=' + response[i].qu_name + '&qu_id=' + response[i].qu_id + '&qu_total_withoutgst=' + response[i].qu_total_withoutgst + '&qu_total_withgst=' + response[i].qu_total_withgst + '&qu_total_gst=' + response[i].qu_total_gst + '&amp;qu_advance=' + response[i].qu_advance + '&qu_balance=' + response[i].qu_balance + '&total_charges=' + response[i].totalcharges + '"><span class="fa fa-eye"></span></a></div></td></tr>';
...
function submitQuotation(type) {
    ...
    else if ($('#qu_order_ref').val() == '' || $('#qu_order_ref').val() == '--Select--') {
    ...
    var data = {
        qu_id: $('#qu_id').val(),
        qu_c_id: $('#qu_c_id').val(),
        qu_order_ref: $('#qu_order_ref').val(),
        qu_due_date: $('#qu_due_date').val(),
        qu_invoice_date: $('#qu_invoice_date').val(),
        ...
        qu_advance: $('#qu_advance').val(),
        qu_balance: $('#qu_balance').text(),
        ...
        quotationInvoice: orderDataArray
    };
    $.ajax({
        type: 'POST',
        url: '/Quotation/Create', // Use the form's action attribute
        data: data, // Serialize the form data
        success: function (response) {
            if (type == 'Close') {
...
function submittemplate(type) {
    /* event.preventDefault();*/
    var rowCount = $('#dataTable tbody tr').length - 1;
    if ($('#qu_name').val() == '' || $('#qu_name').val() == '--Select--') {
...
$(document).ready(function () {
    ...
    let formattedDate = currentDate.toISOString().slice(0, 10);
    $("#qu_invoice_date").val(formattedDate);
    $("#qu_due_date").val(formattedDate);
    ...
    $("#submit").click(function () {
        ...
        report(customer, fromdate, todate);
    });
    ...
    $("#qu_c_id").change(function () {
        var selectedcustomer = $(this).val();
        customer(selectedcustomer);
    });

    $("#qu_quotationname").change(function () {
        var selectedtemplate = $(this).val();
        Template(selectedtemplate);
    })
    //$("#qu_dtp_charges").val("0");
    //$("#qu_pasting_charges").val("0");
    //$("#qu_framing_charges").val("0");
    //$("#qu_fitting_charges").val("0");
    //$("#qu_installation_charges").val("0");
    //$("#qu_shipping_charges").val("0");
...
function customer(qu_c_id) {
    var data = {
        qu_c_id: qu_c_id
    }
    $.ajax({
        type: 'POST',
        url: '/Quotation/Customer', // Use the form's action attribute
        data: data, // Serialize the form data
        success: function (response) {
            $("#c_openingbalance").text(response.c_openingbalance);
            $("#qu_order_no").val(response.qu_order_no);
        },
        error: function (xhr, status, error) {
            // Handle errors
            // console.error(error);
...
function Template(qu_quotationname) {
    var data = {
        qu_quotationname: qu_quotationname
    }
    $.ajax({
        type: 'POST',
...
function submitgrid() {
    ...
    //        alert("Please enter rate!");
    //        return false;
    //    } else if ($('#qu_width').val() == '' || $('#qu_width').val() == 0) {
    //        alert("Please enter length!");
    //        return false;
    //    } else if ($('#qu_height').val() == '' || $('#qu_height').val() == 0) {
    //        alert("Please enter height!");
...
function calculateTotals() {
    ...
    //$("#qu_withgst").val(parseFloat(totalAmount.toFixed(2)));
    //$("#qu_balance").text(parseFloat(totalAmount));
    $("#qu_rate2").val(totalRate.toFixed(2));
    $("#qu_total_quantity").val(totalQuantity);
    $("#qu_total2").val(totalAmount.toFixed(2));
    $("#qu_total_withoutgst").val(totalAmountwithoutgst.toFixed(2));
    $("#qu_total_igst").val(totalGSTI);
    $("#qu_total_sgst").val(totalGSTS);
    $("#qu_total_cgst").val(totalGSTC);
    $("#qu_total_ugst").val(totalGSTU);
    $("#qu_total_gst").val(totalGST);
    $("#qu_total_withgst").val(totalAmount.toFixed(2));
    $("#qu_withgst").val(totalAmount.toFixed(2));
    $("#qu_balance").text(totalAmount);
...
function rate() {
    ...
    amount.val(parseFloat(rate_amount.toFixed(2)));
}
...
function gst() {
    ...
    $("#qu_complete_total").val(parseFloat(amt.toFixed(2)));
}
...
function assignzero() {
    ...
    if ($('#qu_c_id').val() == '' || $('#qu_c_id').val() == '--Select--') {
        alert("Please select customer name!");
        return false;
    }
    else if ($('#qu_order_ref').val() == '' || $('#qu_order_ref').val() == '--Select--') {
...
function final_total() {
    ...
    var totalgst = $('#qu_total_gst').val() || 0;
    ...
    var totalwithgst = (parseFloat(subtotal) + parseFloat(total_dtp_transport)) + (parseFloat(totalgst));
    if (advance == "0" && discount == "0") {
        ...
        //$("#qu_total_withgst").val(totalwithgst);
...
function Excel() {
    window.location.href = '/Quotation/Excel';
}

function Pdf() {
    window.location.href = '/Quotation/Pdf';
}
...
//    var qu_desc = $rowToEdit.find('td:eq(1)').text();
//    var qu_product_name = $rowToEdit.find('td:eq(2)').text();
//    var qu_height = $rowToEdit.find('td:eq(3)').text();
//    var qu_width = $rowToEdit.find('td:eq(4)').text();
//    var qu_size = $rowToEdit.find('td:eq(5)').text();
//    var qu_rate = $rowToEdit.find('td:eq(6)').text();
//    var qu_samount = $rowToEdit.find('td:eq(7)').text();
//    var qu_quantity = $rowToEdit.find('td:eq(8)').text();
//    var qu_total = $rowToEdit.find('td:eq(9)').text();
//    var qu_igst = $rowToEdit.find('td:eq(10)').text();
//    var qu_sgst = $rowToEdit.find('td:eq(11)').text();
//    var qu_ugst = $rowToEdit.find('td:eq(12)').text();
//    var qu_cgst = $rowToEdit.find('td:eq(13)').text();
//    var qu_igstp = $rowToEdit.find('td:eq(14)').text();
//    var qu_sgstp = $rowToEdit.find('td:eq(15)').text();
//    var qu_ugstp = $rowToEdit.find('td:eq(16)').text();
//    var qu_cgstp = $rowToEdit.find('td:eq(17)').text();
//    var qu_gst = $rowToEdit.find('td:eq(18)').text();
//    var qu_CompleteTotal = $rowToEdit.find('td:eq(19)').text();
//    var qu_hsn = $rowToEdit.find('td:eq(20)').text();
...
//    $("#qu_hsn").val(qu_hsn.trim());
//    $("#qu_unit").val(qu_unit.trim());
...
$(document).on('click', '#dataTable tbody .click-edit', function () {
    ...
    var qu_igst = $rowToEdit.find('td:eq(11)').text();
    var qu_sgst = $rowToEdit.find('td:eq(12)').text();
    var qu_ugst = $rowToEdit.find('td:eq(13)').text();
    var qu_cgst = $rowToEdit.find('td:eq(14)').text();
