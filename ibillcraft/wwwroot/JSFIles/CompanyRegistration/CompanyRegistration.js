    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.outcomeDetail == "Company registred successfully!") {
                ...
                var contact = $('#com_contact').val();
                // var country = $('#CountryId').find(":selected").text();
                var country = $('#CountryId').val();
                var url = '/Subscription?ComId=' + comId + '&com_name=' + name + '&com_email=' + email + '&country=' + country + '&contact=' + contact;
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.outcomeDetail == "Subscription inserted successfully!") {
                ...
                //var url = '/Subscription/Payment?ComId=' + comId + '&SubId=' + subId + '&com_company_name=' + com_name + '&com_company_email=' + com_email;
...
function ValidateDetails() {
    ...
    //    return false;
    //} else
        //if ($('#com_code').val() == '' && $('#Contact_no').val() == '') {
    ...
    $.ajax({
        type: 'POST',
        url: '/CompanyLoginRegistration/Create',
        data: data,
        success: function (response) {

            var com_id = response.com_id;
            let isFirstTimeLogin = response.count; // Assuming the response contains this information
            ...
            {
                if (isFirstTimeLogin < 2 && response.rolename=="Admin") {
...
function forgot() {
    var com_code = $('#code').val();
    var Contact_no = $('#username').val();

    //if (com_code == "" || com_code == null) {
    //    alert("Please enter company code !");
    //    return false;
    //} else
    ...
    $.ajax({
        type: 'Get',
        url: '/CompanyLoginRegistration/ForgotPassword',
        data: data,
        success: function (response) {

            if (response == "Please enter valid credentials" || response == "Your subscription has been expired!") {
                alert(response);
                window.location.reload();
            }
            else {
                alert("Password has been send to you email!")
                var url = '/CompanyloginRegistration';//?com_id=' + com_id;// + '&Server_Key=' + name;
                window.location.href = url;
            }
...
function validateNumchar1(event) {
   ...
   else if (!((key == 8) || (key == 32) || (key == 46) || (key >= 35 && key <= 40) || (key >= 65 && key <= 90) || keyk === "0" || keyk === "Tab")) {
        if (event.shiftKey ||
...
function validateMobile1(mobilenumber) {
    var input = mobilenumber.value;
    var result = regex1.test(input);
    if (!result) { // Check if result is false
        alert("Input contains invalid characters.");
        // Optionally clear the input
        $("#Contact_no").val("");
    }
}
function validateMobile2(mobilenumber) {
    var key = mobilenumber.keyCode;
 if (key == 13) {
        ValidateDetails();
    }
}
