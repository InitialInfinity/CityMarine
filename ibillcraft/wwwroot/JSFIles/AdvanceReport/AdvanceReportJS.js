    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.item2.length; i++) {
                htmlrpt += '<tr><td>' + response.item2[i].sl_invoice_no + '</td><td>' + (response.item2[i].sl_invoice_date ? response.item2[i].sl_invoice_date.substring(0, 10) : '') + '</td><td>' + response.item2[i].sl_name + '</td><td>' + response.item2[i].sl_balance + '</td><td>' + response.item2[i].sl_total_withgst + '</td></tr>';
            ...
            for (var i = 0; i < response.item3.length; i++) {
                htmlrpt1 += '<tr><td>' + response.item3[i].pu_invoice_no + '</td><td>' + (response.item3[i].pu_invoice_date ? response.item3[i].pu_invoice_date.substring(0, 10) : '') + '</td><td>' + response.item3[i].pu_name + '</td><td>' + response.item3[i].pu_balance + '</td><td>' + response.item3[i].pu_total_withgst + '</td></tr>';
            }
            ...
            for (var i = 0; i < response.item4.length; i++) {
                htmlrpt2 += '<tr><td>' + (parseFloat(i) + 1) + '</td><td>' + (response.item4[i].e_date ? response.item4[i].e_date.substring(0, 10) : '') + '</td><td>' + response.item4[i].e_category_name + '</td><td>' + response.item4[i].e_user_name + '</td><td>' + response.item4[i].e_amount +'</td></tr>';
            }
            ...
            for (var i = 0; i < response.item5.length; i++) {
                htmlrpt3 += '<tr><td>' + response.item5[i].sp_invoice + '</td><td>' + (response.item5[i].sp_date ? response.item5[i].sp_date.substring(0, 10) : '') + '</td><td>' + response.item5[i].sp_name + '</td><td>' + response.item5[i].sp_pay + '</td><td>' + response.item5[i].sp_balance + '</td></tr>';
            }
