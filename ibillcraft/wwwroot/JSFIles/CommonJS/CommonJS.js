...
function isNumber(event) {
    ...
    //if ((num > 95 && num < 106) || (num > 36 && num < 41) || allowedKeys.includes(num)) {
    //    return; // Allow number keys, arrow keys, delete, backspace, and tab
    //}
    ////if (event.ctrlKey || event.altKey) {
    ////    event.preventDefault(); // Prevent shift, ctrl, alt combinations
    ////} else {
    //    var charCode = event.which || event.keyCode;
    //    var charTyped = String.fromCharCode(charCode);
    //    //if (!/\d/.test(charTyped) && !allowedChars.includes(charTyped)) {
